...
class DownloadController extends Controller
{
    public function index($shared_id)
    {
        ...
        return view('frontend.file.download', ['fileEntry' => $fileEntry, 'downloadLink' => $downloadLink, 'blogArticles' => $blogArticles]);
    }
    ...
    public function createDownloadLink(Request $request, $shared_id)
    {
        ...
        return response()->json([
            'type' => 'success',
            'download_link' => route('file.download.approval', [hashid($downloadLink->id), $fileEntry->shared_id, $fileEntry->name]),
        ]);
    }

    public function download($id, $shared_id, $filename)
    {
        $fileEntry = FileEntry::where('shared_id', $shared_id)->notExpired()->firstOrFail();
        abort_if(!static::accessCheck($fileEntry), 404);
        if (!is_null($fileEntry->password)) {
